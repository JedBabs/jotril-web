...
> Last updated: 2026-07-02. **Session 2026-07-02 (scan retry-storm fix):** diagnosed the "scan fails repeatedly then succeeds even with Spaces running" beta report. Root cause: `/api/gradio-proxy` **buffered** the Gradio SSE poll (`await hfResponse.text()`) — Vercel kills an Edge Function that hasn't started responding within ~25s, and under scan load (up to 90 concurrent queries, 30/Space, on serial free-CPU Spaces) tail-chunk polls legitimately wait longer than that → 504 → `queryJotrilModel` resubmitted on another Space (old event still queued) → retry storm → sweeper churn until concurrency downscaled enough to fit; then `_adaptConcurrencyToNetwork()` reset the cap to 90 on the NEXT scan, re-learning by failing every time. **Fixes:** (1) proxy now **streams** the upstream body (headers+heartbeats flow immediately, no 25s kill); (2) `jotrilService` bounds the poll body read with `POLL_STREAM_TIMEOUT_MS` (110s, under the Edge 120s proxied ceiling) instead of the 30s header timeout killing queued-but-healthy waits; (3) queue-manager persists a sweeper-learned `_learnedCap` across jobs (÷1.5 on engagement, ×1.25 recovery per clean job). Verified: tests 17/17, build clean, live-prod probes (analyze 200, all 3 Spaces healthy solo ~3-4s, 8-concurrency full pipeline 96/96 windows, 192/192 proxy 200s). Prior update: 2026-07-01. **Session 2026-07-01 (pre-beta audit hardening, beta-ops tooling, auth-flow UX) — see new §22.** In short: (1) **Independent pre-launch audit** — confirmed no new code blockers; the binding launch risk is operational (`prisma db push` for the beta schema + Resend DNS + prod env). (2) **Fixed** the abandoned-scan **budget-reservation leak** (new `POST /api/budget/reconcile`, `cancelJob` returns proxyCalls, `useAnalyze` releases on cancel/attribution-failure), **keep-awake** (fail-closed in prod w/o `CRON_SECRET`; warms via submit-only `pingJotrilModels` to dodge the fn timeout), **FK-500s** (feedback P2003→detach, scan-results P2003→401), and made `admin/users` aggregate points DB-side (`groupBy`) instead of loading every `QuotaUsage` row. (3) **Admin broadcast email** — new `/admin/broadcast` composer + `POST/GET /api/admin/broadcast` (ADMIN) + `sendBulkEmails`/`renderBroadcastHtml` (Resend batch, individually addressed). (4) **Feedback screenshots** — `Feedback.screenshot` (base64 data URL; **needs `prisma db push`**), widget attach w/ client downscale, admin lazy-load lightbox. (5) **Email `from` 422 fix** — `resolveFrom()` strips stray env quotes + validates. (6) **Auth-flow UX** — new `POST /api/auth/resend-verification` + sign-in "resend" button; verify-email now shows a friendly "already used" state (+ StrictMode double-fire guard); Google button gated behind the Terms checkbox on signup (+ implicit-consent line on sign-in). (7) **GlitchFavicon** now updates every icon link (defeats Next's SVG preference) + Safari node-reinsertion. ⚠️ **Manual steps outstanding:** `npx prisma db push` (beta `User`/`Feedback` cols **+ new `Feedback.screenshot`**), add `https://www.jotril.com/api/auth/callback/google` to the Google OAuth client + confirm prod `NEXTAUTH_URL=https://www.jotril.com`, Resend SPF/DKIM. **Beta launch scaffolding (2026-06-30):** added the private-beta machinery for the first 50 Covenant University testers — see new **§21**. In short: (1) **Email moved to Resend** (`src/lib/email.js` now prefers the Resend HTTP API via `RESEND_API_KEY` → SMTP → mock; branded templates + a new beta-Pro-activated email); SMTP was previously unconfigured (mock-only). (2) **CU beta Pro comp** — `src/lib/beta.js` gates `@stu.cu.edu.ng`, caps at 50 testers, grants PRO for 2 months on email confirmation (hooked into `/api/auth/verify-email`); `User` gained `roleExpiresAt`/`betaTester`/`betaGrantedAt`; **expiry is enforced live in the NextAuth `session` callback via `effectiveRole()`** (no cron) so Pro auto-reverts to FREE — admin manual tier changes clear the expiry. (3) **Feedback system** — new `Feedback` model + `POST /api/feedback` (guest-OK) + admin `GET /api/admin/feedback` & `PATCH/DELETE /api/admin/feedback/[id]`; global `FeedbackWidget` (floating button, mounted in `Providers`) + `/admin/feedback` triage page (linked from the admin hub). `ToastContainer` is now mounted **once globally** in `Providers` (removed from the 4 per-page mounts). (4) **Legal** — `/legal/terms`, `/legal/privacy`, `/legal/beta` pages, footer links + beta badge + a global results-accuracy disclaimer (footer + every `HeatmapViewer` result), and a Terms/Privacy consent checkbox on signup. ⚠️ **Two manual steps remain (NOT done by code):** run `npx prisma db push` to create the `Feedback` table + new `User` columns (blocked from auto-run as a prod-DB migration), and set `RESEND_API_KEY` + verify the sending domain (SPF/DKIM) in DNS. Verified: build exit 0, tests 17/17, new-file lint clean, beta logic unit-checked. **Security + correctness audit (2026-06-29):** fixed a critical `/api/gradio-proxy` open-relay/SSRF `HF_TOKEN` leak (hostname allow-list + HMAC scan-token cookie), the doubly-broken `/api/v1/detect` public API, the soft budget reconcile (now counts real proxy calls + charges overage), plaintext API keys (now hashed), host-header injection in email links, missing security headers, and per-request middleware logging; parallelized the analyze hot-path gates; added QuotaUsage composite indexes (needs `prisma db push`); deleted dead `queryJotrilBatch`; fixed the 4 stale tests (suite now 17/17). All verified (build/tests/lint/token/SSRF). Full detail in §15 (Fixed 2026-06-29) + §8. **Also 2026-06-29 (polish + fixes):** softened multimodal marketing copy to "AI text detection (more modalities coming)"; deleted the inert queue tier-sort; made the landing page reachable while logged in (removed forced `/`→`/dashboard` redirect, added a navbar "Home" link); cleaned ~51 root scratch files (kept 6 dev tools); fixed the "sign in to upload" bug for signed-in users (`FileUploader` now derives auth from `useSession`) + replaced the toast with a designed sign-in overlay; fixed the `admin.jotril.com` subdomain auth (env-gated `COOKIE_DOMAIN` cross-subdomain session cookie + absolute admin redirect — needs `COOKIE_DOMAIN=.jotril.com` in prod). **Earlier this session (2026-06-28):** documented a previously-undocumented marketing/SEO/analytics/network-resilience layer that landed after the 2026-06-24 doc (new `/text` route, sitemap/robots/OG, GA4 + Vercel Analytics/Speed Insights, `resilient-fetch`, offline pause, ServiceWorker, GlitchFavicon, subdomains) — see new §20 and the refreshed §4 map. Also **corrected the queue tier-priority claim**: the QueueManager tier sort is effectively inert — it's a per-browser singleton (users never compete) AND the UI passes no real tier (FileUploader → useAnalyze defaults `userTier=1`), so every chunk enters at tier 1; only the sweeper's `tier:999` retry preemption ever fires. Real tier differentiation is server-side (governor depth + quota), NOT the queue — see §9. ⚠️ Marketing copy now positions Jotril as multi-modal (text/image/video/audio/deepfake) but the engine is **TEXT-ONLY** — a positioning-vs-capability gap to close before launch. **Prior, 2026-06-24 (AUTO-TUNER LIFECYCLE FIX:** stuck-run deadlock + undefined-metrics-on-deadline, see §15. Added `TuningRun.updatedAt` heartbeat (`prisma db push` — RESTART dev server for the new client); `/run` POST + SSE poller now reclaim a no-heartbeat "active" run after 2 min instead of jamming the dataset; `runExhaustiveSearch` early-deadline returns now include train/test/full metrics via `finalizeResult()`. Also fixed: score-cache poisoning guard (`MAX_NULL_RATE` abort) + force-rebuild path & "Rebuild Cache" UI; `/dev-trigger` shares the run lifecycle (no in-memory lock, keeps COMPLETE history); "best MCC"→"best obj" relabel; `apply` preserves the budget block. Earlier 2026-06-24: HF SPACE LOG CLEANUP: silenced Gradio's Starlette `HTTP_422_UNPROCESSABLE_ENTITY` deprecation warning via a scoped `warnings.filterwarnings` at the top of `app.py` in all three Spaces — committed + pushed (Space-3 was cloned in; it wasn't local). Earlier 2026-06-24: HIGH-FIDELITY DOCX REPORT CACHE + AUTO-PREWARM via Gotenberg + GCS, see §15/§19. New: lib/gotenberg.js, lib/report-storage.js, lib/report/server-overlay.js, /api/report/prewarm, /api/report/download. DOCX scans now prewarm in the background (DOCX→PDF→highlights+cover→GCS) so fresh AND history downloads are instant high-fidelity. Persisted-scan downloads use a real `<a download>` navigation to the new GET endpoint — survives IDM/FDM (which was eating fetch+blob into a fake 204 the day before — see §16). Verified Node-side: Gotenberg IAM, GCS round-trip, server overlay. Prior: user-cancellable processes; load-time fixes; PDF report engine rebuilt on headless Chrome — see §19. Plan: Hobby now → 50-tester private beta → Pro before public launch.
...
DEV_PIN                 Dev-admin PIN — LOCAL DEV ONLY. The dev-PIN login is hard-disabled when NODE_ENV=production (no admin backdoor in prod) and has no hardcoded fallback (must be set explicitly to work in dev). The "Developer Access" toggle is likewise dropped from the sign-in UI in prod builds. Prod admin is via a real account with role=ADMIN.
...
onScanComplete(windowResults) callback [useAnalyze.js]:
  ...
  - POST /api/attribute { sentences, scenarios, scores, budgetToken, executedQueries, actualInvocations }
    ↳ budgetToken = HMAC-signed { estimate, monthKey, callsPerQuery } issued by /api/analyze.
      The governor internals are NOT sent as plain fields (kept off the wire) and can't be
      forged by the client. /api/analyze returns { scenarios, sentences, sourceHtml, filename,
      chunkCount, budgetToken } — depth/estimate/monthKey/callsPerQuery no longer exposed.
      Abandon path POST /api/budget/reconcile { budgetToken, actualInvocations } (same token).
      Sign/verify: lib/scan-token.js signBudgetToken/verifyBudgetToken.
  ↓
POST /api/attribute [src/app/api/attribute/route.js]  (pure CPU, ~ms)
  ...
  - verifyBudgetToken → reconcileScan: refund (estimate − actual) into UsageBudget
...
### Fixed 2026-07-02 (scan retry-storm under load — the "fails multiple times then succeeds" beta report)
- **🚩 `/api/gradio-proxy` buffered the SSE poll body → Vercel's ~25s Edge initial-response limit 504'd every slow poll.** Gradio's `/gradio_api/call/predict/<eid>` GET is a long-held SSE stream (heartbeats) that only closes when the Space's queue reaches the event. The proxy did `await hfResponse.text()` before responding, so nothing was sent until the event completed — any poll behind >25s of Space queue died with a platform 504 even though the Space was healthy. At scan concurrency (90 workers, 30/Space, free CPU Spaces processing ~serially) tail chunks *always* waited that long → mass 504s → `queryJotrilModel` treated the Space as dead and **resubmitted on the next Space** (the original event still queued on the first, so load compounded) → chunks burned 5 retries + 3 sweeper passes while concurrency downscaled ÷1.5 per engagement until the storm fit. **Fix:** proxy streams `hfResponse.body` — headers + heartbeats forward immediately, no buffering, no 25s kill.
- **Client poll timeout restructured.** With streaming, `secureFetch`'s 30s `PROXY_REQUEST_TIMEOUT_MS` only bounds connection setup (fetch resolves on headers); the SSE body read is now bounded by `POLL_STREAM_TIMEOUT_MS = 110s` (`readBodyWithTimeout`, cancels the stream) — long queue waits are waited out instead of killed-and-resubmitted; only a genuinely wedged stream (past the Edge ~120s proxied-request ceiling) is abandoned.
- **Queue concurrency learning persisted across scans.** `_adaptConcurrencyToNetwork()` reset `MAX_CONCURRENCY` to the full pool (90) on every `enqueueJob`, so each scan re-learned the sustainable level by failing. New `_learnedCap`: set to the downscaled value on sweeper engagement (jobs marked `swept`), recovers ×1.25 per clean job completion, caps the base in `_adaptConcurrencyToNetwork`.
- **Verified:** tests 17/17, `next build` clean; live-prod probes: `/api/analyze` 200 (3.9s cold/~1s warm), all 3 Spaces healthy solo (~3-4s/query), full-pipeline 8-concurrency probe through prod proxy = 96/96 windows, 192/192 proxy calls 200. **Needs a redeploy to take effect; hard-refresh clients (QueueManager singleton).**
...
## 21. Beta Program, Email (Resend), Feedback & Legal (2026-06-30)

The private-beta launch layer for the first 50 Covenant University testers.

### Email — now Resend (`src/lib/email.js`)
Delivery priority: **Resend HTTP API** (`RESEND_API_KEY`, a plain HTTPS POST — preferred on Vercel serverless, no SMTP socket to cold-start/get egress-blocked) → **SMTP** via Nodemailer (`EMAIL_SERVER_*`; works with Resend SMTP too: host `smtp.resend.com`, port 465, user `resend`, pass = API key) → **mock** (logs to server console). FROM = `EMAIL_FROM`. Templates share a branded `layout()` shell with a BETA badge + the results-accuracy/beta disclaimer in the footer. Emails: `sendVerificationEmail`, `sendPasswordResetEmail`, and new `sendBetaProEmail(email, baseUrl, expiresAt)`. **Pre-2026-06-30 reality:** SMTP env was empty/placeholder → all mail was mock-only. ⚠️ **Setup required:** create a Resend key, set `RESEND_API_KEY`, and verify the sending domain (SPF + DKIM) or mail to real inboxes (CU = Google Workspace) lands in spam/bounces.

### Beta Pro comp (`src/lib/beta.js`)
- Constants: `BETA_EMAIL_DOMAIN='stu.cu.edu.ng'`, `BETA_MAX_TESTERS=50`, `BETA_PRO_MONTHS=2`.
- `isBetaEmail(email)` — `endsWith('@stu.cu.edu.ng')` (rejects `…stu.cu.edu.ng.com` spoofs).
- `grantBetaProIfEligible(prisma, user)` — idempotent; skips non-CU / already-beta / ADMIN / ULTRA; respects the 50 cap (`count({betaTester:true})`); sets `role:'PRO'`, `roleExpiresAt = now+2mo`, `betaTester:true`, `betaGrantedAt`. Returns `{granted, reason, expiresAt}`.
- `effectiveRole(user)` — returns FREE when `roleExpiresAt` is in the past; otherwise the stored role. **Policy: open signup (anyone can register, stays FREE); CU emails auto-get Pro on email confirmation.**
- **Hook points (two, both idempotent via `grantBetaProIfEligible`):**
  - **Credentials signup:** `/api/auth/verify-email` calls the grant after marking `emailVerified`, then fires `sendBetaProEmail` (best-effort; never blocks verification). The route response includes `{betaPro, betaReason}`; the verify-email page celebrates a Pro grant.
  - **Google OAuth:** Google sign-in never hits verify-email (Google pre-verifies the email), so the grant also runs in the **NextAuth `jwt` callback** OAuth branch — it re-reads the DB user at sign-in (after the Prisma adapter has created/linked the user, so the row + id exist), attempts the grant, updates `token.role`/`roleExpiresAt`, and sends the email. Idempotent: a credentials user already granted at verification returns `already_beta` (no re-grant, no duplicate email). Runs only on actual sign-ins (jwt `if (user)`).
- **Admin beta-slot view:** `/api/admin/users` GET returns `stats.betaTesters` (`count({betaTester:true})`) + `stats.betaCap` (`BETA_MAX_TESTERS`); the admin hub renders a progress banner ("N / 50 claimed" + slots-remaining + bar) above the analytics cards.
- **Expiry enforcement = NextAuth `session` callback** (`[...nextauth]/route.js`): token carries `role` + `roleExpiresAt`; the session callback runs `effectiveRole()` every request (no DB hit, no cron) so an expired grant reads as FREE everywhere `session.user.role` is consumed (incl. `/api/analyze` tier → governor depth, quota). `authorize()` + the jwt callback now thread `roleExpiresAt`. **Admin manual tier change clears `roleExpiresAt`** (`/api/admin/users` PATCH) so an admin grant is permanent.
- **Caveat:** the JWT only refreshes role/expiry from DB at sign-in (30-day token), same as before — a mid-session grant by admin needs re-login or a session `update()`. Expiry downgrade IS live (computed from the token's `roleExpiresAt`, not re-read).

### Feedback system
- **Model `Feedback`** — `userId?` (SetNull), `email?`, `category` (bug|idea|complaint|praise|other), `message` (Text), `rating?` (1-5), `pageUrl?`, `userAgent?`, `status` (NEW|IN_PROGRESS|RESOLVED|WONTFIX), `adminNote?`. Indexed on status/createdAt/userId.
- **`POST /api/feedback`** — open to everyone; attaches `userId`/email from session when present, else a guest-provided email. Validates message (≤4000) + category whitelist.
- **`GET /api/admin/feedback`** (ADMIN) — `?status=` filter, cursor pagination (50/page), status counts via `groupBy`. **`PATCH`/`DELETE /api/admin/feedback/[id]`** (ADMIN) — triage status/note, or delete spam.
- **`FeedbackWidget.jsx`** — floating bottom-right launcher + glass modal (category chips, message, optional guest email, 1-5 stars). Mounted globally in `Providers`; hidden on `/admin/*`.
- **`/admin/feedback/page.jsx`** — triage page: status filter tabs w/ counts, cards with one-click status buttons + delete + load-more. Linked from the admin hub header ("Feedback" button).
- **ToastContainer centralised:** now mounted ONCE in `Providers` (so global `showToast` works on every page incl. the landing + the widget); removed from the 4 per-page mounts (`admin`, `dashboard`, `dashboard/api-keys`, `text`) to avoid duplicate toasts.

### Legal / disclaimers
- Pages: `/legal/terms`, `/legal/privacy`, `/legal/beta` (shared `components/LegalLayout.jsx` shell). Beta-appropriate copy; **flag for a lawyer review before public launch.**
- Footer (`page.js`): legal links + BETA badge + a global results-accuracy disclaimer ("probabilistic estimate, not proof, not the sole basis for academic/disciplinary decisions").
- `HeatmapViewer` renders the same accuracy disclaimer under every result.
- Signup: a BETA + CU-perk banner, a live "you'll get Pro free" hint when the email is `@stu.cu.edu.ng`, and a **required Terms/Privacy consent checkbox** (submit disabled until checked).

### Schema delta (needs `prisma db push` — NOT run; blocked as a prod-DB migration)
- `User`: `+ roleExpiresAt DateTime?`, `+ betaTester Boolean @default(false)`, `+ betaGrantedAt DateTime?`, `+ feedbacks Feedback[]`.
- `+ model Feedback` (above). Purely additive (new nullable columns + new table) — safe.

### New env
`RESEND_API_KEY` (Resend HTTP API key; unset → SMTP/mock). `EMAIL_FROM` must be on a verified domain. `NEXT_PUBLIC_APP_URL` is reused for beta/verification email links in prod.

---

## 22. Pre-Beta Audit Hardening, Beta-Ops Tooling & Auth-Flow UX (2026-07-01)

A deep pre-launch pass: an independent audit (no new code blockers found), fixes for the real-but-non-blocking issues it surfaced, two beta-ops features (admin broadcast email + feedback screenshots), and a run-through of the sign-up flow with UX fixes. **Verified each step: build "Compiled successfully", tests 17/17, changed/added files ESLint-clean.**

### Audit outcome (independent of this doc)
The launch risk is **operational, not code**: (a) the beta schema **`prisma db push`** must be run or all auth breaks (User queries select the new columns → P2022); (b) Resend SPF/DKIM; (c) prod env (`NEXTAUTH_SECRET`, `NEXTAUTH_URL`, `HF_TOKEN`, `RESEND_API_KEY`, `CRON_SECRET`, Google OAuth). Verified-good: proxy SSRF allow-list + scan-token gate, hashed API keys, bcrypt/brute-force/verify-gate auth, secrets gitignored, security headers.

### Budget-reservation leak — FIXED (`/api/analyze` reserve never released on abandonment)
`resolveScan` reserves invocations up-front; only `/api/attribute` reconciled. A cancel / attribution-failure / tab-close left the reservation stuck in `UsageBudget.used` → over a month it inflates and the governor throttles depth prematurely. **Fix:** new **`POST /api/budget/reconcile`** (thin, clamped wrapper over `reconcileScan`; same trust model as `/api/attribute`). `QueueManager.cancelJob(jobId)` now **returns the job's `proxyCalls`** so the client refunds only the *unused* portion. `useAnalyze` holds a `budgetRef` ({estimate, monthKey}) + a one-shot idempotent `reconcileBudget()` fired on cancel (real spent count) and on attribution-failure (`meta.proxyCalls`); the success path nulls the ref (server already reconciled) so it can't double-refund. **Residual (accepted):** cancel *during* the `/api/analyze` request itself (before the client learns monthKey/estimate) + hard tab-close still can't reconcile client-side — both brief, far smaller than the original leak.

### keep-awake — FIXED (`/api/cron/keep-awake`)
(a) **Fail-closed in production** when `CRON_SECRET` is unset (503, runs nothing) — it triggers HF inference, so an unprotected one is a liability; dev still runs freely. (b) Warms via **`pingJotrilModels()`** (submit-only, doesn't poll the 30-60s cold-start inference) instead of full `queryJotrilModel` → can't exceed the serverless fn timeout. The submit still reaches each Space's inference endpoint, so the 48h sleep timer still resets. **Requires `CRON_SECRET` set in prod** or the cron 503s and Spaces eventually sleep.

### FK-500s on a stale JWT — FIXED
A 30-day JWT can outlive a deleted user row. `POST /api/feedback` now catches **P2003** (FK) → re-saves detached (`userId:null`) and **P2022** (screenshot column not pushed) → saves without it. `POST /api/scan-results` maps **P2003** → clean 401 (the client save is best-effort) instead of a 500.

### admin/users perf — FIXED
Was loading **every** `QuotaUsage` row for every user into memory to sum points (grows unbounded). Now a single DB-side `prisma.quotaUsage.groupBy({ by:['userId'], _sum:{pointsCost} })` + a lookup map. `requestsMade` still uses the correlated `_count`.

### Admin broadcast email (NEW)
Send an announcement to registered users from the admin panel — **each recipient individually addressed** (never a shared To/CC).
- **`src/lib/email.js`:** `renderBroadcastHtml({heading,message,footerNote})` (plain text → paragraphs, HTML-escaped, branded shell; `footerNote` reserved for a post-beta unsubscribe line, currently empty) + `sendBulkEmails(messages)` (Resend **`/emails/batch`**, ≤100/req, paced 600ms between chunks, failures counted not thrown; falls back to sequential `sendEmail` for SMTP/mock). Returns `{total,sent,failed,errors}`.
- **`POST/GET /api/admin/broadcast`** (ADMIN): GET = recipient counts per audience (`Promise.allSettled`, resilient); POST = send, or `{test:true}` sends one copy to `testEmail`/the admin. Audiences `all | verified | beta | pro`. ⚠️ **`User.email` is a required (non-nullable) column — do NOT filter `email:{not:null}`** (Prisma rejects `not:null` on a non-nullable field → the counts 500'd; this was the "Failed to load audience counts" bug). `emailVerified` IS nullable so it can use `not:null`.
- **`/admin/broadcast/page.jsx`:** audience picker w/ live counts, subject/heading/message, live email preview, **Send test to me**, **Send to N** behind a confirm. Linked from the admin hub header (`Mail` icon, next to Feedback).

### Feedback screenshots (NEW — needs `prisma db push`)
- **Schema:** `Feedback.screenshot String? @db.Text` (base64 data URL). **Additive; `prisma db push` required.** Until pushed, feedback WITH an image still saves (drops the image via the P2022 fallback) and the admin list tolerates the missing column.
- **`FeedbackWidget.jsx`:** "Attach screenshot" → client-downscales (`fileToDownscaledDataUrl`, max 1280px JPEG q0.7, capped ~1.4MB) → thumbnail + remove.
- **`POST /api/feedback`:** validates a `data:image/...;base64,` URL ≤1.5MB.
- **Admin:** the base64 is **kept OUT of the list payload** — `GET /api/admin/feedback` uses an explicit `select` (no `screenshot`) + a cheap `id`-only `NOT:{screenshot:null}` probe → `hasScreenshot`; the blob is lazy-loaded via a **new `GET /api/admin/feedback/[id]`** and shown in a lightbox on the triage page.

### Email `from` 422 — FIXED (`src/lib/email.js`)
Resend rejected sends with `422 Invalid 'from' field` because the env value reached it with literal wrapping quotes. **`resolveFrom()`** strips wrapping quotes, trims, validates against `email@domain` / `Name <email@domain>`, and falls back to `Jotril AI <noreply@jotril.com>`. (Also removed a stray empty `EMAIL_FROM=""` duplicate in `.env.local`.) The Resend domain `jotril.com` is verified (sending enabled). **Gotcha:** worked in local dev but failed in prod → the fix is now env-shape-agnostic so it's correct everywhere.

### Auth-flow UX (sign-up "should be easy" pass)
- **Resend verification** — new **`POST /api/auth/resend-verification`** (no user-enumeration; always returns generic success; reuses `createEmailVerificationToken`). The sign-in page shows a **"Resend verification email"** button ONLY when sign-in fails with the "verify your email" error (the #1 signup dead-end: lost/spam email). Signup success copy points to spam + the resend path.
- **verify-email page** — a consumed/expired token (re-click / refresh) now renders a friendly **"This link's already been used → Sign In"** state instead of the scary red "Invalid Link" (real errors still red). Added a `ranRef` guard so React StrictMode's double-invoke can't POST the one-time token twice + overwrite a success.
- **Google + Terms consent** — `Continue with Google` on **signup** is now gated behind the same required Terms/Privacy checkbox (it's an account-creation path); the **sign-in** Google button (also creates accounts for new users) got an implicit-consent line ("By continuing with Google, you agree to…").
- **Google OAuth config (NOT code):** `redirect_uri_mismatch` in prod was the Google client missing **`https://www.jotril.com/api/auth/callback/google`** (list had bare `jotril.com`, but canonical is `www` per `NEXT_PUBLIC_APP_URL`). Fix = add the `www` callback URI + set prod **`NEXTAUTH_URL=https://www.jotril.com`** (NextAuth derives the callback host from it).
- **🚩 Google OAuth had NEVER worked end-to-end — `User` was missing `image`.** After the redirect_uri fix, Google completed but bounced back to sign-in (`?error=OAuthCreateAccount`): NextAuth's `@auth/prisma-adapter` `createUser` writes `{name,email,emailVerified,image}`, but the `User` model had no `image` column → Prisma "Unknown argument `image`" → `createUser` throws → no session → middleware bounces off `/dashboard`. **Fix:** added `image String?` to `User` (schema valid; `prisma db push` run → column live; **confirmed working** — account creates, session lands on dashboard, CU beta grant fires). Only the credentials path had ever been exercised. Lesson: any NextAuth Prisma-adapter User needs `image`.
